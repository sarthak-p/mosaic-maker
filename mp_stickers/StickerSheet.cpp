...
void StickerSheet::removeSticker (unsigned index) {
    for (unsigned i = 0; i < max_; i++) {
        if ((index < max_) && (arr[index] != nullptr)) {
            delete arr[index];
            arr[index] = nullptr;
        }
    }
...
Image StickerSheet::render() const {
    ...
    for (unsigned i = 0; i < max_; i++) {
        ...
        if (arr[i] != nullptr) {
            highest = (y_[i] + arr[i]->height());
            widest = (x_[i] + arr[i]->width());
        }
        if (height_ < highest) {
            height_ = highest; 
        }
    ...
    for (unsigned i = 0; i < max_; i++) {
        if (_.i) {
            for(unsigned w = x_[i]; w < (arr[i].width() + x_[i]); w++) {
                for(unsigned h = y_[i]; h < (arr[i].height() + y_[i]); h++) {
                    HSLAPixel & pixel = base.getPixel(w, h);
                    HSLAPixel & picture_pixel = arr[i].getPixel(w - x_[i], h - y_[i]);
